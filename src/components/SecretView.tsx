...
export default function SecretView({ initialPush, token, isLocked = false }: Props) {
  ...
  async function fetchWithPassphrase() {
    ...
    if (!res.ok || data.error) {
      setPassphraseError('Nesprávné heslo nebo přístup odepřen.')
      return
    }
    setPush(data)
  ...
  if ((push?.expired || push?.deleted) || deleted) {
    return (
      <div className="rounded-lg border border-[var(--border-col)] bg-[var(--card-bg)] p-6 text-center space-y-2">
        ...
        <p className="font-medium text-[var(--fg)]">Tajemství expiralo</p>
        <p className="text-sm text-[var(--muted-col)]">Tento odkaz již není platný nebo byl smazán.</p>
        <a href="/" className="inline-block mt-2 text-sm text-brand hover:underline">Vytvořit nový push →</a>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {needsPassphrase && (
        <div className="rounded-lg border border-[var(--border-col)] bg-[var(--card-bg)] p-6 text-center space-y-3">
          ...
          <p className="font-medium text-[var(--fg)]">Chráněno heslem</p>
      ...
      {!needsPassphrase && push?.payload && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-[var(--fg)]">Tajemství</span>
            {revealed && (
              <button
                onClick={copy}
        ...
        <div className="flex flex-wrap gap-3 text-xs text-[var(--muted-col)]">
          {push.views_remaining !== undefined && (
            <span className="flex items-center gap-1">
              ...
              {push.views_remaining} zobrazení zbývá
            </span>
          )}
          {push.days_remaining !== undefined && (
            <span className="flex items-center gap-1">
              ...
              vyprší za {push.days_remaining} {push.days_remaining === 1 ? 'den' : 'dní'}
            </span>
          )}
